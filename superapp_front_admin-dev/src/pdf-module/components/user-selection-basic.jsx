...
import { getInternalUsers } from '@/services/UsersService';
...
const UserSelectionBasic = ({
  ...
  const [dummyDynamicUserData, setDynamicUserData] = useState([
    // Dynamic users
    {
      userId: '${recipient001}',
      value: '${recipient001}',
      label: 'destinatario001',
      email: 'destionatario@gmail.com',
      dynamic: true,
    },
  ]);
  ...
  useEffect(() => {
    const fetchResponsiblesUser = async () => {
      try {
        ...
        const responsibleUsersRespond = await getInternalUsers(decodedToken.acc_id);
        setResponsibleUsers(
          responsibleUsersRespond.data.data
        );
      } catch (error) {
  ...
  const addUser = () => {
    ...
    const newUser = {
      ...
      label: `destinatario${newUserNumberString}`,
      email: `destinatario${newUserNumberString}@gmail.com`,
      dynamic: true,
    };
...
export default UserSelectionBasic;
